    ...
    if (photo.url || photo.image) {
        ...
        if (photo.url) {
            ...
            [[SDWebImageManager sharedManager] diskImageExistsForURL:photo.url completion:^(BOOL isInCache) {
                ...
                [weakSelf.imageView sd_setImageWithURL:photo.url placeholderImage:photo.imageView.image options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize) {
